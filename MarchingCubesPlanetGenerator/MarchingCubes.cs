...
public class MarchingCubes
{
  ...
  private float densidadCubos = 1;
  private float treshold;
  private float shift;
  private Vector3[] verticesCuboLocal;
  private Value value;
  private static float epsilon = 5f;
  
  public MarchingCubes(float densidadCubos, MeshBuilder mesh, float treshold, Value value)
  {
    this.densidadCubos = densidadCubos;
    this.mesh = mesh;
  ...
  private void recalcularCuboLocal()
  {
    shift = densidadCubos / 2;
    verticesCuboLocal = new Vector3[8]{
  ...
  public void setDensidadCubos(float densidad)
  {
    densidadCubos = densidad;
    recalcularCuboLocal();
  }

  public void analizarMarchingCube(Vector3 centroCubo)
  {
  ...
  private Vector3[] calcularVertCuboEnPunto(Vector3 punto)
  {
    ...
    for (int i = 0; i < 8; i++)
      cuboEnPunto[i] = verticesCuboLocal[i] + punto;
